 ...
 */
@ApplicationScoped
public class ManualSubscriptionCommandFromMessageExtractor implements SubscriptionCommandFromMessageExtractor {
    ...
    public Stream<Command> extractCommands(String representation, SenderCriterion ignoredSenderCriterion) {
        AssetPageRetrievalMessage assetPageRetrievalMessage = AssetPageRetrievalMessage.decodeManualSubscriptionMessage(representation);
        SubscriptionEntity manualSubscription = subscriptionFinder.findSubscriptionById(assetPageRetrievalMessage.getSubscriptionId());

        List<AssetEntity> assets = assetPageRetrievalMessage.isGroup() ?
                handleAssetGroup(assetPageRetrievalMessage) : handleMainAssets(assetPageRetrievalMessage, manualSubscription.getAssets());
        ...
        return Stream.concat(nextPageCommandStream, makeCommandsForManualSubscription(manualSubscription, assets));
    }

    private Stream<Command> makeCommandsForManualSubscription(SubscriptionEntity manualSubscription, List<AssetEntity> assets) {
        return assets.stream()
                .distinct()
                .map(a -> new AssetAndSubscriptionData(a, manualSubscription))
                .map(this::makeTriggeredSubscriptionEntity)
                .map(this::makeCommandForManualSubscription);
    }
    ...
    private Command makeCommandForManualSubscription(TriggeredSubscriptionEntity triggeredSubscription) {
