 ...
 * <pre>{@code
 *     @Schedule(hour = "*", minute = "*", second = "* /5", info = "activatePendingSubscriptionExecutions", persistent = true)
...
public class SchedulerEjb {
	...
	@Schedule(hour = "*", minute = "*/2", info = "activatePendingSubscriptionExecutions")
	public void activatePendingSubscriptionExecutions() {
		log.debug("Subscription SchedulerEjb started");
		subscriptionSchedulerService.activatePendingSubscriptionExecutions(dateTimeService.getNowAsDate());
		log.debug("Subscription SchedulerEjb finished");
	}
}
