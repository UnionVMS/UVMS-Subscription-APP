...
/**
 * Command to trigger a subscription.
 */
@Vetoed
class TriggerSubscriptionCommand implements Command {
	...
	public TriggerSubscriptionCommand(Function<TriggeredSubscriptionEntity, Set<TriggeredSubscriptionDataEntity>> extractTriggeredSubscriptionDataForDuplicates, TriggeredSubscriptionService triggeredSubscriptionService, SubscriptionExecutionScheduler subscriptionExecutionScheduler, SubscriptionExecutionService subscriptionExecutionService, TriggeredSubscriptionEntity triggeredSubscription) {
	...
	private boolean notAlreadyTriggered(TriggeredSubscriptionEntity triggeredSubscription) {
		return Optional.of(triggeredSubscription)
				.map(extractTriggeredSubscriptionDataForDuplicates)
				.map(criteria -> !triggeredSubscriptionService.isDuplicate(triggeredSubscription, criteria))
				.orElse(false);
	}
}
